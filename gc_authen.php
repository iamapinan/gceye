...
Author: Apinan Woratrakun <iamapinan@gmail.com>
Version: 1.0
...
function gc_authen($user = NULL, $username = '', $password = '') {
    ...
    if($mode == 'production') {
		...
		//$bind = @ldap_bind($ldap, "$domain\\z0007xxx", 'xxxxxxxx');
    }
    ...
    // Create log files to http://gceye.pttgcgroup.com/wp-content/plugins/gc_authen/log/
    // file_put_contents(__DIR__ .'/log/'.$username.'.log', json_encode($info) );
    // Skip group check
    // if(($info[0]['memberof']['count'] !== 0 && count($filterGroup) !== 0)){
    //     // $selectGroup = explode(",", array_values($filterGroup)[0]);
    //     $info['role'] = 'guest'; // $roles[explode("=", $selectGroup[0])[1]];
    // } else {
    //     $user = new WP_Error( 'authentication_failed', __( '<strong>ERROR</strong>: You are not allow to access GC EYE services.!' ) );
    //     return $user;
    // }
